...
__align(32) int memtbl[MEMTBLSIZE]__attribute__((at(0x20000100 + MEMBASESIZE))); //用int可以支持更多内存块，short仅能支持3w+个；正数表示未分配，负数表示已分配
...
void mem_init(void){
    ...
    splay_insert(&data);
    for(i = 0; i < MEMTBLSIZE; ++i) memtbl[i] = 0;
...
void* my_malloc(unsigned int sz){ //sz单位：字节
    ...
    if(suc->data.cnt == inf) return (void*)0; //查询到splay的右边界，即没有足够大的内存块
